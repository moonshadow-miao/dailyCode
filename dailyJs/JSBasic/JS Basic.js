...
// 函数式编程：1 是一种编程范式，常见编程范式：面向对象，面向过程，函数式编程。
//            2 JS中，函数是一等公民，体现在函数可以赋值给变量，可以作为函数参数，函数返回值。（函数作为函数的参数和返回值，称为高阶函数，高阶函数式函数式编程的基础）
//            3 闭包：一个函数和对其周围状态（词法环境）的引用捆绑在一起称为闭包

// 纯函数：函数的返回结果只依赖参数，相同输入始终得到相同输出，函数执行过程没有副作用（函数执行中，产生对外部可观察的变化就是副作用，例如改变外部变量，依赖外部变量）
    // 1 可缓存
    // 2 可测试

// 函子：把异常和异步操作等副作用控制在可控的范围之内。
    // 例如
    class Functor {
        constructor(value) {
            this.value = value
